#!/usr/bin/env bash
# ============================================================================
# SVote-2S — Run a Single Evaluation Stage
# ============================================================================
# Usage:
#   bash scripts/run_stage.sh <STAGE> <MODEL> <DATASET> [NUM_GPUS]
#
# Stages:
#   sc         — P0. SC Sampling (N=20 stochastic CoT)
#   cisc       — P1. CISC P(True) on SC samples
#   rasc       — P2. RASC (CPU, no LLM)
#   svote      — P3. SVote Stage-1 + Stage-2 (requires P0 + P1)
#   cisc_s2    — P4. CISC on SVote Stage-2 (requires P3)
#   fusion     — P5. SVote-2S Fusion grid search (requires P0,P1,P3,P4)
#   cot        — P6. CoT (deterministic decoding)
#   rvsc       — P7. Ranked Voting SC
#
# Examples:
#   bash scripts/run_stage.sh sc      qwen2_5_7b_instruct hotpotqa 4
#   bash scripts/run_stage.sh cisc    qwen2_5_7b_instruct hotpotqa 4
#   bash scripts/run_stage.sh svote   qwen2_5_7b_instruct hotpotqa 4
#   bash scripts/run_stage.sh cot     gemma2_9b_it musique 4
#   bash scripts/run_stage.sh fusion  llama3_1_8b_instruct 2wiki_dev12576
# ============================================================================
set -euo pipefail

STAGE="${1:?Usage: $0 <STAGE> <MODEL> <DATASET> [NUM_GPUS]}"
MODEL="${2:?Usage: $0 <STAGE> <MODEL> <DATASET> [NUM_GPUS]}"
DATASET="${3:?Usage: $0 <STAGE> <MODEL> <DATASET> [NUM_GPUS]}"
NUM_GPUS="${4:-4}"

PROJECT="$(cd "$(dirname "$0")/.." && pwd)"
PYTHON="${PYTHON:-/GPUdata/student/asc002/ouyangguo/anaconda/envs/vetagent/bin/python}"
CONFIG="$PROJECT/configs/models.yaml"
EVAL_CONFIG="${EVAL_CONFIG:-$PROJECT/configs/eval_default.yaml}"

# ---------------------------------------------------------------------------
# Read eval hyperparameters from eval_default.yaml
# ---------------------------------------------------------------------------
yq() {
    $PYTHON -c "
import yaml
with open('$EVAL_CONFIG') as f: cfg = yaml.safe_load(f)
keys = '$1'.split('.')
v = cfg
for k in keys: v = v[k]
print(v)
"
}

# Model-specific
declare -A DTYPE_MAP=( [qwen2_5_7b_instruct]=float16 [gemma2_9b_it]=bfloat16 [llama3_1_8b_instruct]=float16 )
DTYPE="${DTYPE_MAP[$MODEL]:-float16}"
MAX_NEW_TOKENS=$(yq "max_new_tokens.$MODEL")

# Sampling
N_SAMPLES=$(yq "sampling.n_samples")
TEMPERATURE=$(yq "sampling.temperature")
TOP_P=$(yq "sampling.top_p")
MAX_MODEL_LEN=$(yq "sampling.max_model_len")
SEED=$(yq "sampling.seed")

# SVote
K_MIN=$(yq "svote_stage1.k_min")
K_MAX=$(yq "svote_stage1.k_max")
N_RESAMPLE=$(yq "svote_stage2.n_resample")
WEIGHT_MODE=$(yq "svote_stage1.weight_mode")
CONTEXT_MODE=$(yq "svote_stage1.context_mode")

# RVSC
RVSC_TOP_K=$(yq "rvsc.top_k")

# Directories
OUT_ROOT="$PROJECT/outputs/$MODEL/$DATASET"
LOG_ROOT="$OUT_ROOT/logs"
P0_DIR="$OUT_ROOT/stage0_samples"
P1_DIR="$OUT_ROOT/cisc_stage0"
P3_DIR="$OUT_ROOT/svote_stage12"
P4_DIR="$OUT_ROOT/cisc_stage2"
MERGED="$P0_DIR/merged"

mkdir -p "$LOG_ROOT"

echo "=================================================================="
echo " Stage: $STAGE | model=$MODEL dataset=$DATASET gpus=$NUM_GPUS"
echo " max_new_tokens=$MAX_NEW_TOKENS dtype=$DTYPE"
echo "=================================================================="

# ---------------------------------------------------------------------------
# Helper: parallel multi-GPU launch + wait
# ---------------------------------------------------------------------------
wait_all() {
    local pids=("$@")
    for i in "${!pids[@]}"; do
        wait "${pids[$i]}" || { echo "[ERROR] GPU $i FAILED (exit $?)"; exit 1; }
    done
}

merge_jsonl() {
    local pattern="$1" outfile="$2" sortkey="${3:-id}"
    $PYTHON -c "
import glob, json
rows = []
for p in sorted(glob.glob('$pattern')):
    with open(p) as f:
        for line in f:
            if line.strip(): rows.append(json.loads(line))
rows.sort(key=lambda r: r.get('$sortkey',''))
with open('$outfile', 'w') as f:
    for r in rows: f.write(json.dumps(r, ensure_ascii=False) + '\n')
print(f'Merged {len(rows)} records -> $outfile')
"
}

# ====================================================================
case "$STAGE" in
# ====================================================================
sc)
    echo "[$(date)] SC Sampling (N=$N_SAMPLES) ..."
    mkdir -p "$P0_DIR"
    LOG_DIR="$LOG_ROOT/p0_sc_sampling"; mkdir -p "$LOG_DIR"

    PIDS=()
    for GPU in $(seq 0 $((NUM_GPUS-1))); do
        CUDA_VISIBLE_DEVICES=$GPU $PYTHON "$PROJECT/src/eval/sample_pc_score_vllm.py" \
            --dataset "${DATASET}_shard${GPU}" \
            --model "$MODEL" \
            --setting fullctx \
            --run_id "shard${GPU}" \
            --config "$CONFIG" \
            --output_root "$P0_DIR" \
            --n_samples $N_SAMPLES \
            --temperature $TEMPERATURE \
            --top_p $TOP_P \
            --max_new_tokens $MAX_NEW_TOKENS \
            --max_model_len $MAX_MODEL_LEN \
            --dtype "$DTYPE" \
            --seed $SEED \
            --overwrite \
            > "$LOG_DIR/shard${GPU}.log" 2>&1 &
        PIDS+=($!)
    done
    wait_all "${PIDS[@]}"

    # Merge
    mkdir -p "$MERGED"
    $PYTHON -c "
import glob, json
files = sorted(glob.glob('$P0_DIR/shard*/${DATASET}_shard*/${MODEL}/fullctx/samples.jsonl'))
rows = []
for p in files:
    with open(p) as f:
        for line in f:
            if line.strip(): rows.append(json.loads(line))
rows.sort(key=lambda r: r['id'])
with open('$MERGED/samples.jsonl', 'w') as f:
    for r in rows: f.write(json.dumps(r, ensure_ascii=False) + '\n')
print(f'Merged {len(rows)} questions from {len(files)} shards')
"
    echo "[$(date)] SC Sampling complete. -> $MERGED/samples.jsonl"
    ;;

# ====================================================================
cisc)
    echo "[$(date)] CISC P(True) on SC samples ..."
    mkdir -p "$P1_DIR"
    LOG_DIR="$LOG_ROOT/p1_cisc"; mkdir -p "$LOG_DIR"

    PIDS=()
    for GPU in $(seq 0 $((NUM_GPUS-1))); do
        SHARD_SAMPLES="$P0_DIR/shard${GPU}/${DATASET}_shard${GPU}/${MODEL}/fullctx/samples.jsonl"
        [ -f "$SHARD_SAMPLES" ] || { echo "[ERROR] $SHARD_SAMPLES not found. Run 'sc' first."; exit 1; }
        CUDA_VISIBLE_DEVICES=$GPU $PYTHON "$PROJECT/src/eval/eval_cisc_ptrue_vllm.py" \
            --samples_jsonl "$SHARD_SAMPLES" \
            --dataset "${DATASET}_shard${GPU}" \
            --model "$MODEL" \
            --config "$CONFIG" \
            --output_dir "$P1_DIR/shard${GPU}" \
            --max_model_len $MAX_MODEL_LEN \
            --dtype "$DTYPE" \
            --overwrite \
            > "$LOG_DIR/shard${GPU}.log" 2>&1 &
        PIDS+=($!)
    done
    wait_all "${PIDS[@]}"

    merge_jsonl "$P1_DIR/shard*/per_question_cisc.jsonl" "$P1_DIR/merged_per_question_cisc.jsonl"
    echo "[$(date)] CISC complete."
    ;;

# ====================================================================
rasc)
    echo "[$(date)] RASC (CPU post-processing) ..."
    P2_DIR="$OUT_ROOT/rasc"
    [ -f "$MERGED/samples.jsonl" ] || { echo "[ERROR] $MERGED/samples.jsonl not found. Run 'sc' first."; exit 1; }
    $PYTHON "$PROJECT/src/baselines/rasc/eval_rasc.py" \
        --samples_jsonl "$MERGED/samples.jsonl" \
        --output_dir "$P2_DIR" \
        --overwrite \
        > "$LOG_ROOT/p2_rasc.log" 2>&1
    echo "[$(date)] RASC complete. -> $P2_DIR/"
    ;;

# ====================================================================
svote)
    echo "[$(date)] SVote Stage-1 + Stage-2 ..."
    mkdir -p "$P3_DIR"
    LOG_DIR="$LOG_ROOT/p3_svote"; mkdir -p "$LOG_DIR"

    PIDS=()
    for GPU in $(seq 0 $((NUM_GPUS-1))); do
        SHARD_SAMPLES="$P0_DIR/shard${GPU}/${DATASET}_shard${GPU}/${MODEL}/fullctx/samples.jsonl"
        SHARD_CISC="$P1_DIR/shard${GPU}/per_question_cisc.jsonl"
        [ -f "$SHARD_SAMPLES" ] || { echo "[ERROR] $SHARD_SAMPLES not found. Run 'sc' first."; exit 1; }
        [ -f "$SHARD_CISC" ] || { echo "[ERROR] $SHARD_CISC not found. Run 'cisc' first."; exit 1; }
        CUDA_VISIBLE_DEVICES=$GPU $PYTHON "$PROJECT/src/svote/eval_svote_vllm.py" \
            --samples_jsonl "$SHARD_SAMPLES" \
            --cisc_perq "$SHARD_CISC" \
            --dataset "${DATASET}_shard${GPU}" \
            --model "$MODEL" \
            --config "$CONFIG" \
            --output_dir "$P3_DIR/shard${GPU}" \
            --weight_mode $WEIGHT_MODE \
            --k_min $K_MIN --k_max $K_MAX \
            --context_mode $CONTEXT_MODE \
            --n_resample $N_RESAMPLE \
            --temperature $TEMPERATURE \
            --top_p $TOP_P \
            --max_new_tokens $MAX_NEW_TOKENS \
            --max_model_len $MAX_MODEL_LEN \
            --dtype "$DTYPE" \
            --seed $SEED \
            --overwrite \
            > "$LOG_DIR/shard${GPU}.log" 2>&1 &
        PIDS+=($!)
    done
    wait_all "${PIDS[@]}"

    merge_jsonl "$P3_DIR/shard*/per_question.jsonl" "$P3_DIR/merged/per_question.jsonl"
    echo "[$(date)] SVote Stage-1/2 complete."
    ;;

# ====================================================================
cisc_s2)
    echo "[$(date)] CISC on SVote Stage-2 samples ..."
    mkdir -p "$P4_DIR"
    LOG_DIR="$LOG_ROOT/p4_cisc_s2"; mkdir -p "$LOG_DIR"

    PIDS=()
    for GPU in $(seq 0 $((NUM_GPUS-1))); do
        S2_SAMPLES="$P4_DIR/shard${GPU}_samples.jsonl"
        PQ="$P3_DIR/shard${GPU}/per_question.jsonl"
        [ -f "$PQ" ] || { echo "[ERROR] $PQ not found. Run 'svote' first."; exit 1; }
        $PYTHON -c "
import json
with open('$PQ') as f:
    rows = [json.loads(l) for l in f if l.strip()]
with open('$S2_SAMPLES', 'w') as f:
    for r in rows:
        out = {'id': r['id'], 'gold_answer': r['gold_answer'],
               'samples': r.get('samples', []), 'question': ''}
        f.write(json.dumps(out, ensure_ascii=False) + '\n')
print(f'Prepared {len(rows)} stage-2 questions for CISC shard $GPU')
"
        CUDA_VISIBLE_DEVICES=$GPU $PYTHON "$PROJECT/src/eval/eval_cisc_ptrue_vllm.py" \
            --samples_jsonl "$S2_SAMPLES" \
            --dataset "${DATASET}_shard${GPU}" \
            --model "$MODEL" \
            --config "$CONFIG" \
            --output_dir "$P4_DIR/shard${GPU}" \
            --max_model_len $MAX_MODEL_LEN \
            --dtype "$DTYPE" \
            --overwrite \
            > "$LOG_DIR/shard${GPU}.log" 2>&1 &
        PIDS+=($!)
    done
    wait_all "${PIDS[@]}"

    merge_jsonl "$P4_DIR/shard*/per_question_cisc.jsonl" "$P4_DIR/merged_per_question_cisc.jsonl"
    echo "[$(date)] CISC Stage-2 complete."
    ;;

# ====================================================================
fusion)
    echo "[$(date)] SVote-2S Fusion grid search ..."
    P5_DIR="$OUT_ROOT/fusion"
    mkdir -p "$P5_DIR"

    [ -f "$MERGED/samples.jsonl" ]                       || { echo "[ERROR] Run 'sc' first."; exit 1; }
    [ -f "$P1_DIR/merged_per_question_cisc.jsonl" ]      || { echo "[ERROR] Run 'cisc' first."; exit 1; }
    [ -f "$P3_DIR/merged/per_question.jsonl" ]           || { echo "[ERROR] Run 'svote' first."; exit 1; }
    [ -f "$P4_DIR/merged_per_question_cisc.jsonl" ]      || { echo "[ERROR] Run 'cisc_s2' first."; exit 1; }

    T_VALUES=$(yq "fusion.T_values" | tr -d '[],' | tr ',' ' ')
    ALPHA_VALUES=$(yq "fusion.alpha_values" | tr -d '[],' | tr ',' ' ')
    BEST_EM=0; BEST_T=""; BEST_ALPHA=""

    for T in $T_VALUES; do
        for ALPHA in $ALPHA_VALUES; do
            OUT="$P5_DIR/T${T}_alpha${ALPHA}.json"
            $PYTHON "$PROJECT/src/svote/fusion_cisc2.py" \
                --stage1_samples "$MERGED/samples.jsonl" \
                --stage2_perq "$P3_DIR/merged/per_question.jsonl" \
                --stage1_cisc "$P1_DIR/merged_per_question_cisc.jsonl" \
                --stage2_cisc "$P4_DIR/merged_per_question_cisc.jsonl" \
                --T_s1 "$T" --T_s2 "$T" --alpha_s2 "$ALPHA" \
                --output "$OUT" > /dev/null 2>&1

            EM=$($PYTHON -c "import json; d=json.load(open('$OUT')); print(d['em'])")
            IS_BETTER=$($PYTHON -c "print(1 if $EM > $BEST_EM else 0)")
            if [ "$IS_BETTER" = "1" ]; then
                BEST_EM="$EM"; BEST_T="$T"; BEST_ALPHA="$ALPHA"
            fi
            echo "  T=$T α=$ALPHA EM=$EM"
        done
    done

    cp "$P5_DIR/T${BEST_T}_alpha${BEST_ALPHA}.json" "$P5_DIR/best.json"
    echo "{\"best_T\": $BEST_T, \"best_alpha\": $BEST_ALPHA, \"best_em\": $BEST_EM}" > "$P5_DIR/best_params.json"
    echo "[$(date)] Fusion complete. Best: T=$BEST_T α=$BEST_ALPHA EM=$BEST_EM"
    ;;

# ====================================================================
cot)
    echo "[$(date)] CoT (temperature=0, deterministic) ..."
    P6_DIR="$OUT_ROOT/cot"
    LOG_DIR="$LOG_ROOT/p6_cot"; mkdir -p "$P6_DIR" "$LOG_DIR"

    PIDS=()
    for GPU in $(seq 0 $((NUM_GPUS-1))); do
        CUDA_VISIBLE_DEVICES=$GPU $PYTHON "$PROJECT/src/eval/sample_pc_score_vllm.py" \
            --dataset "${DATASET}_shard${GPU}" \
            --model "$MODEL" \
            --setting fullctx \
            --run_id "shard${GPU}" \
            --config "$CONFIG" \
            --output_root "$P6_DIR" \
            --n_samples 1 \
            --temperature 0.0 \
            --top_p 1.0 \
            --max_new_tokens $MAX_NEW_TOKENS \
            --max_model_len $MAX_MODEL_LEN \
            --dtype "$DTYPE" \
            --seed $SEED \
            --overwrite \
            > "$LOG_DIR/shard${GPU}.log" 2>&1 &
        PIDS+=($!)
    done
    wait_all "${PIDS[@]}"
    echo "[$(date)] CoT complete. -> $P6_DIR/"
    ;;

# ====================================================================
rvsc)
    echo "[$(date)] Ranked Voting SC ..."
    P7_DIR="$OUT_ROOT/rvsc"
    LOG_DIR="$LOG_ROOT/p7_rvsc"; mkdir -p "$P7_DIR" "$LOG_DIR"

    PIDS=()
    for GPU in $(seq 0 $((NUM_GPUS-1))); do
        CUDA_VISIBLE_DEVICES=$GPU $PYTHON "$PROJECT/src/baselines/rvsc/eval_rvsc_vllm.py" \
            --dataset "${DATASET}_shard${GPU}" \
            --model "$MODEL" \
            --setting fullctx \
            --run_id "shard${GPU}" \
            --config "$CONFIG" \
            --output_root "$P7_DIR" \
            --n_samples $N_SAMPLES \
            --top_k $RVSC_TOP_K \
            --max_new_tokens $MAX_NEW_TOKENS \
            --temperature $TEMPERATURE \
            --top_p $TOP_P \
            --max_model_len $MAX_MODEL_LEN \
            --dtype "$DTYPE" \
            --seed $SEED \
            --overwrite \
            > "$LOG_DIR/shard${GPU}.log" 2>&1 &
        PIDS+=($!)
    done
    wait_all "${PIDS[@]}"
    echo "[$(date)] RVSC complete. -> $P7_DIR/"
    ;;

# ====================================================================
*)
    echo "Unknown stage: $STAGE"
    echo "Available stages: sc, cisc, rasc, svote, cisc_s2, fusion, cot, rvsc"
    exit 1
    ;;
esac

echo "=================================================================="
echo " Done: $STAGE | $MODEL / $DATASET"
echo "=================================================================="
